...
def generateEllipsoidVoxels(filename):
    ...
    y_voxels = pd.DataFrame([[insideEllipsoid(Y.iloc[j, :6], gridpoint) or insideEllipsoid(Y.iloc[j, 6:], gridpoint) 
    for gridpoint in gridpoints] for j in range(len(Y))])
    ...
    filename1 = 'VoxelsDataset/2ellipsoids/'+os.path.basename(filename)+'.csv'

    return df_vox.to_csv(filename1, index=False)


files =  os.listdir('TwoEllipsoids2')
files.sort()

for filename in files:
    generateEllipsoidVoxels('TwoEllipsoids2/'+filename)
